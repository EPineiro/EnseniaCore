...
class VideoProcessor {
  ...
  public:
	  ...
	  //Si se abre un archivo con un video conviene setear el delay como 1000 / fps para que se vea a la velocidad normal
	  //Si se abre una camara conectada a la compu, conviene setear el delay a un numero especifico.
	  void setDelay(int d) {
	  ...
	  //Sirve para guardar un frame especifico al presionar la tecla indicada (codigo ASCII de la tecla)
	  void setKeyToCaptureFrame(int key) {

		  this->keyToCaptureFrame = key;
	  }
	  ...
	  void run() {
		  ...
		  while (!isStopped()) {
			  ...
			  if(key == keyToCaptureFrame) {

				  //guardamos el output como un archivo
				  std::stringstream captureName;
				  captureName<<"captura"<<captureIndex<<".jpg";
				  cv::imwrite(captureName.str(), output);
				  captureIndex++;
			  }

			  //Salimos del programa si el usuario presiona ESC (codigo ASCII 27)
			  if (key == 27)
				  stopIt();
